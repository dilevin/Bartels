...
# include<../include/linear_tri2dmesh_arap_dq2.h>
#endif
...
void sim::linear_tri2dmesh_arap_dq2(Eigen::SparseMatrix<DerivedRet, Options, StorageIndex> &out, const Eigen::MatrixBase<DerivedV> &V,  Eigen::Ref<const Eigen::MatrixXi> E,
    ...
    auto assemble_func = [&q, &func](auto &H,  auto &e, 
                           ...
                           { 
                             linear_tri2d_arap_dq2(H, q, e, sim::unflatten<3,2>(dphidX), params, volume(0));
                             func(H); //callback stuff
                           };
    

    Eigen::Matrix6x<DerivedRet> Htmp;
    sim::assemble(out, 2*V.rows(), 2*V.rows(), E, E, assemble_func, Htmp, dphidX, volume, params);
}
